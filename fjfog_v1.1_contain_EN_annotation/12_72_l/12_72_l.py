'''Extract 12-72 level prediction'''
import numpy as np
np.random.seed(1337)
import keras
...
absu = a+'/'
fsock = open(absu+'12_72_l_error.log', 'a')
sys.stderr = fsock
sa = tttt.strftime("%Y%m%d%H%M", tttt.localtime())
ef = open(absu+'12_72_l_error.log', 'a')
ef.write('*****************'+str(sa)+'*****************'+'\n')
ef.close()
start = tttt.clock()
